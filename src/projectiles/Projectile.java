...
public abstract class Projectile extends EntityImpl {
	...
	public void update() {
		...
		if (ticks == tickspeed) {
			...
			if (friendly)
				es = Model.model.intersectsCreeps(hitbox);
			else
				es = Model.model.intersectsFriendly(hitbox);
	...
	/**
	 * helper function that calculates and applies unit movement ratios so we don't have to always copy out the fucking trig.
	 */
